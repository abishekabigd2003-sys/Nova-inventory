...
$Body = @{ name = 'Test User'; email = 'testuser2@inventory.com'; password = 'password123' }
$JsonBody = $Body | ConvertTo-Json
try {
    Invoke-RestMethod -Uri 'http://localhost:5000/api/auth/register' -Method Post -Body $JsonBody -ContentType 'application/json'
} catch {
...
Write-Host 'Testing Dashboard Data...'
$DashRes = Invoke-RestMethod -Uri 'http://localhost:5000/api/dashboard' -Method Get -Headers $Headers
...
$ProdBody = @{ name = "Test Product $Rand"; sku = "TST-$Rand"; categoryId = $CatId; price = 100; minStockLevel = 5; description = 'A test product' }
